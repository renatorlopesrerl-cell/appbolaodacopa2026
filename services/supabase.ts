...
// Custom fetch com timeout de 45s e sem cache para todas as chamadas diretas ao Supabase
const fetchWithTimeout = (input: RequestInfo | URL, init?: RequestInit) => {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 45000); // 45s timeout

    // Use cache: 'no-store' to prevent Android WebView from caching Supabase responses.
    // NOTE: We do NOT send Cache-Control or Pragma as request headers because Supabase's
    // CORS policy does not whitelist them, which would cause preflight failures.
    return fetch(input, {
        cache: 'no-store',
        ...init,
