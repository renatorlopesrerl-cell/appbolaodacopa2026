...
var __awaiter2, resolveFetch3, resolveHeadersConstructor, fetchWithAuth;
...
var init_SupabaseAuthClient = __esm({
  "../node_modules/@supabase/supabase-js/dist/module/lib/SupabaseAuthClient.js"() {
    ...
    init_checked_fetch();
    init_module3();
    SupabaseAuthClient = class extends GoTrueClient_default {
      static {
...
var __awaiter3, DEFAULT_GLOBAL_OPTIONS, DEFAULT_DB_OPTIONS, DEFAULT_AUTH_OPTIONS, DEFAULT_REALTIME_OPTIONS, SupabaseClient;
...
function errorResponse(error, statusOverride) {
  const status = statusOverride || (error.message === "Unauthorized" ? 401 : error.message === "Forbidden" ? 403 : error.message === "Service Unavailable" ? 503 : 500);
...
function getUserClient(env, request) {
...
async function sendPushNotificationToUser(env, userId, title, body, data) {
...
var init_shared = __esm({
  "api/_shared.ts"() {
    ...
    __name(sendPushNotificationToUser, "sendPushNotificationToUser");
  }
});

// api/admin/leagues.ts
var onRequest;
var init_leagues = __esm({
  "api/admin/leagues.ts"() {
    init_functionsRoutes_0_25198053012443267();
    init_checked_fetch();
    init_shared();
...
var init_matches = __esm({
  "api/admin/matches.ts"() {
    ...
    init_checked_fetch();
    init_shared();
    onRequest2 = /* @__PURE__ */ __name(async ({ request, env, data }) => {
      try {
        const userClient = getUserClient(env, request);
        if (request.method === "POST") {
          ...
          if (oldMatch && updates.status && updates.status !== oldMatch.status) {
            ...
            if (updates.status === "IN_PROGRESS") {
              title = "Jogo Iniciado! \u26BD";
              bodyText = `A partida entre ${home} x ${away} come\xE7ou!`;
            } else if (updates.status === "FINISHED") {
              title = "Fim de Jogo! \u{1F3C1}";
              bodyText = `Resultado final: ${home} ${updates.home_score} x ${updates.away_score} ${away}`;
            }
            if (title) {
              const supabase = getSupabaseClient(env);
              const { data: profiles } = await supabase.from("profiles").select("id, notification_settings").not("fcm_token", "is", null);
              if (profiles) {
...
// api/leagues/invites.ts
var getLeagueLimit, onRequest3;
var init_invites = __esm({
  "api/leagues/invites.ts"() {
    init_functionsRoutes_0_25198053012443267();
    init_checked_fetch();
    init_shared();
    getLeagueLimit = /* @__PURE__ */ __name((settings) => {
    ...
    onRequest3 = /* @__PURE__ */ __name(async ({ request, env, data }) => {
      try {
        ...
        if (request.method === "GET") {
          ...
          const { data: data2, error } = await userClient.from("league_invites").select("*").eq("email", email.toLowerCase()).eq("status", "pending");
          if (error) throw error;
        ...
        if (request.method === "POST") {
          ...
          if (action === "invite") {
            ...
            if (existingUser && league.participants.includes(existingUser.id)) {
              return errorResponse(new Error("Usu\xE1rio j\xE1 participa desta liga"), 400);
            }
            const { data: existingInvite } = await userClient.from("league_invites").select("id").eq("league_id", leagueId).eq("email", email.toLowerCase()).eq("status", "pending").maybeSingle();
            if (existingInvite) {
              return errorResponse(new Error("Convite j\xE1 enviado para este usu\xE1rio"), 400);
            }
            ...
            if (existingUser) {
              sendPushNotificationToUser(
                env,
                existingUser.id,
                "Novo Convite! \u{1F3C6}",
                `Voc\xEA foi convidado para participar da liga: ${league.name}`,
                { url: `/league/${leagueId}` }
          ...
          if (action === "respond") {
            ...
            if (accept) {
              ...
              const updatedParticipants = Array.from(/* @__PURE__ */ new Set([...league.participants, authUser.id]));
              const updatedPending = (league.pending_requests || []).filter((id) => id !== authUser.id);
...
// api/leagues/join.ts
var getLeagueLimit2, onRequest4;
var init_join = __esm({
  "api/leagues/join.ts"() {
    init_functionsRoutes_0_25198053012443267();
    init_checked_fetch();
    init_shared();
    getLeagueLimit2 = /* @__PURE__ */ __name((settings) => {
      if (settings?.isUnlimited) return Infinity;
      const plan = settings?.plan || "FREE";
      switch (plan) {
        case "VIP_UNLIMITED":
    ...
    onRequest4 = /* @__PURE__ */ __name(async ({ request, env, data }) => {
      ...
      try {
        ...
        const limit = getLeagueLimit2(league.settings);
        ...
        if (league.is_private) {
          sendPushNotificationToUser(
            env,
            league.admin_id,
            "Nova Solicita\xE7\xE3o \u{1F514}",
...
// api/leagues/members.ts
var getLeagueLimit3, onRequest5;
var init_members = __esm({
  "api/leagues/members.ts"() {
    init_functionsRoutes_0_25198053012443267();
    init_checked_fetch();
    init_shared();
    getLeagueLimit3 = /* @__PURE__ */ __name((settings) => {
      if (settings?.isUnlimited) return Infinity;
      const plan = settings?.plan || "FREE";
      switch (plan) {
        case "VIP_UNLIMITED":
    ...
    onRequest5 = /* @__PURE__ */ __name(async ({ request, env, data }) => {
      if (request.method !== "POST") return new Response("Method not allowed", { status: 405 });
      try {
        const authUser = data.user;
        const userClient = getUserClient(env, request);
        const { leagueId, userId, action } = await request.json();
        ...
        if (action === "approve") {
          ...
          const limit = getLeagueLimit3(league.settings);
          if (league.participants.length >= limit) return jsonResponse({ success: false, message: "League limit reached" }, 400);
          const updatedPending = league.pending_requests.filter((id) => id !== userId);
          const updatedParticipants = Array.from(/* @__PURE__ */ new Set([...league.participants, userId]));
          await userClient.from("leagues").update({
            participants: updatedParticipants,
            pending_requests: updatedPending
          }).eq("id", leagueId);
...
var init_debug = __esm({
  "api/debug.ts"() {
    init_functionsRoutes_0_25198053012443267();
    init_checked_fetch();
    onRequest7 = /* @__PURE__ */ __name(async ({ env }) => {
      const status = {
...
// api/leagues.ts
var onRequest8;
var init_leagues2 = __esm({
  "api/leagues.ts"() {
    init_functionsRoutes_0_25198053012443267();
    init_checked_fetch();
    init_shared();
    onRequest8 = /* @__PURE__ */ __name(async ({ request, env, data }) => {
      try {
        const url = new URL(request.url);
        const authUser = data.user;
        const userClient = getUserClient(env, request);
        if (request.method === "GET") {
          const data2 = await withRetry(async () => {
            return await userClient.from("leagues").select("*");
          });
          const cleanLeagues = data2?.filter((l) => !l.name.includes("[EXCLU\xCDDA]")) || [];
        ...
        if (request.method === "POST") {
          ...
          const generatedId = `l-${Date.now()}-${Math.floor(Math.random() * 1e3)}`;
          const getLeagueCode = /* @__PURE__ */ __name(() => {
            let code = "";
...
var init_predictions = __esm({
  "api/predictions.ts"() {
    ...
    init_checked_fetch();
    init_shared();
    onRequest10 = /* @__PURE__ */ __name(async ({ request, env, data }) => {
      try {
        const userClient = getUserClient(env, request);
        const authUser = data.user;
        if (request.method === "GET") {
...
var init_profiles = __esm({
  "api/profiles.ts"() {
    ...
    init_checked_fetch();
    init_shared();
    onRequest11 = /* @__PURE__ */ __name(async ({ request, env, data }) => {
      try {
        const authUser = data.user;
        const userClient = getUserClient(env, request);
        if (request.method === "GET") {
          ...
          const id = url.searchParams.get("id");
          if (id) {
            const { data: data3, error } = await userClient.from("profiles").select("*").eq("id", id).single();
...
var init_storage = __esm({
  "api/storage.ts"() {
    ...
    init_checked_fetch();
    init_shared();
    onRequest12 = /* @__PURE__ */ __name(async ({ request, env }) => {
      if (request.method !== "POST") return new Response("Method not allowed", { status: 405 });
      try {
        const userClient = getUserClient(env, request);
        const { base64, folder, fileName } = await request.json();
        ...
        const { error } = await userClient.storage.from(bucketOverride).upload(filePath, bytes, {
          cacheControl: "3600",
          upsert: true,
          contentType
        });
        ...
        const { data: { publicUrl } } = userClient.storage.from(bucketOverride).getPublicUrl(filePath);
...
var init_middleware = __esm({
  "api/_middleware.ts"() {
    ...
    init_module4();
    __name(retry, "retry");
    onRequest13 = /* @__PURE__ */ __name(async ({ request, env, next, data }) => {
...
// ../.wrangler/tmp/pages-oHMAdq/functionsRoutes-0.25198053012443267.mjs
var routes;
var init_functionsRoutes_0_25198053012443267 = __esm({
  "../.wrangler/tmp/pages-oHMAdq/functionsRoutes-0.25198053012443267.mjs"() {
    init_leagues();
    init_matches();
    init_invites();
...
// ../.wrangler/tmp/bundle-ld12HM/middleware-loader.entry.ts
init_functionsRoutes_0_25198053012443267();
init_checked_fetch();

// ../.wrangler/tmp/bundle-ld12HM/middleware-insertion-facade.js
init_functionsRoutes_0_25198053012443267();
init_checked_fetch();
...
//# sourceMappingURL=functionsWorker-0.5269725411178368.mjs.map
