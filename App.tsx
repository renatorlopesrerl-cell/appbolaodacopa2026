...
const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchAllData = async () => {
    ...
    try {
        // 1. LEAGUES
        try {
            const { data: leaguesData, error: lError } = await supabase.from('leagues').select('*');
            if (leaguesData && !lError) {
        ...
        // 2. MATCHES
        try {
            const { data: dbMatches, error: mError } = await supabase.from('matches').select('*');
            if (dbMatches && dbMatches.length > 0 && !mError) {
