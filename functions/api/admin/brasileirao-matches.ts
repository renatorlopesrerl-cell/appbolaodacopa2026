...
export const onRequest = async (context: any) => {
    ...
    try {
        ...
        if (request.method === 'POST') {
            ...
            const { status, is_blocked, home_score, away_score, phase, date, location, group, reminder_30m_sent, home_team_id, away_team_id } = body;
            const safeUpdates: Record<string, any> = {};
            if (status !== undefined)           safeUpdates.status = status;
            if (is_blocked !== undefined)       safeUpdates.is_blocked = is_blocked;
            ...
            const { error } = await userClient.from('brasileirao_matches').update(safeUpdates).eq('id', id);
        ...
        console.error("Brasileirao Match Update Error:", e, "Message:", e.message, "Code:", e.code, "Details:", e.details);
