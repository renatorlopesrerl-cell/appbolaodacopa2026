...
export const BrazilLeagueDetails: React.FC = () => {
    ...
    const { currentUser, brazilLeagues: leagues, matches, brazilPredictions: predictions, users, currentTime, loading, invitations, submitBrazilPredictions, updateBrazilLeague: updateLeague, deleteBrazilLeague: deleteLeague, approveBrazilUser: approveUser, rejectBrazilUser: rejectUser, removeUserFromBrazilLeague: removeUserFromLeague, brazilMatchGoals, addBrazilMatchGoal, addNotification, refreshPredictions, isRefreshingPredictions, joinBrazilLeague: joinLeague, sendBrazilLeagueInvite: sendLeagueInvite, brazilPlayers, loadLeagueData, fetchMatchPredictions, hasWatchedPredictionAd, setHasWatchedPredictionAd, hasWatchedStatsAd, setHasWatchedStatsAd } = useStore();
    ...
    useEffect(() => {
        ...
        if (tabParam === 'classificacao' || tabParam === 'regras' || tabParam === 'admin') {
    ...
    const [userHistoryPredictions, setUserHistoryPredictions] = useState<BrazilPrediction[]>([]);
            ...
            const fetchHistory = async () => {
                ...
                if (data) {
                    setUserHistoryPredictions(data.map(p => ({
                        ...
                        goalscorerPoints: p.goalscorer_points
                    })));
                }
    ...
    const appStateRef = useRef({ pendingEdits, showUnsavedModal, zoomedImage, currentUser, league, activeTab, selectedUserId });
    appStateRef.current = { pendingEdits, showUnsavedModal, zoomedImage, currentUser, league, activeTab, selectedUserId };
    ...
    const leaderboard = useMemo(() => {
        ...
        if (currentPeriodRankings && currentPeriodRankings.length > 0) {
            return league.participants.map(userId => {
                ...
                const user = userFromDb || userFromStore || { name: 'Usuário', id: userId, email: '', avatar: '', isAdmin: false, whatsapp: '', theme: undefined, isPro: false } as User;
    ...
    useEffect(() => {
        if (selectedMatchForDetails && league) {
            ...
            api.matches.getDetailedMatchStats(selectedMatchForDetails, league.id, true)
                .then(data => {
    ...
    useEffect(() => {
        ...
        if (isLocked || match.status === MatchStatus.IN_PROGRESS || match.status === MatchStatus.FINISHED) {
            fetchMatchPredictions(selectedMatchForDetails, league.id, 'brazil');
        }
    }, [selectedMatchForDetails, league?.id]);
    ...
    const renderMatchBadge = (match: Match) => {
        ...
        return (
            <span className={`text-[8px] min-[360px]:text-[9px] min-[380px]:text-[10px] ${colorClass} px-1.5 py-0.5 rounded font-bold border uppercase tracking-wider whitespace-nowrap shrink-0`}>
                {label}
            </span>
        );
    };
    ...
    const getMatchStatsData = (matchId: string, overridePreds?: BrazilPrediction[]) => {
        ...
        const participantPreds = league.participants
            ...
            .filter((p): p is BrazilPrediction => p !== undefined);
    ...
    const renderPalpitesTab = () => {
      ...
      const getMatchDetailsData = () => {
        ...
        const participants = league.participants.map(uid => {
                                ...
                                if (!mLocked) {
                                    if (!isProUser && !hasWatchedStatsAd && Capacitor.isNativePlatform()) {
                                        ...
                                        try {
                                            ...
                                            setApiMatchStats(null);
                                            setLoadingStats(true);
                                            api.matches.getStats(m.id, league.id, 'brazil')
                                            ...
                                            <div className="flex justify-between items-start mb-1">
                                                <div>
                                                    ...
                                                    <p className="text-blue-100 text-[10px] font-bold">
                                                        {detailsData.participants.length} palpite{detailsData.participants.length !== 1 ? 's' : ''} registrado{detailsData.participants.length !== 1 ? 's' : ''}
                                                    </p>
                                                </div>
                                                            ...
                                                            {pagedDetailsParticipants.map(({ user, pred }, idx) => {
                                                                ...
                                                                if (detailsData.match.status === MatchStatus.FINISHED || detailsData.match.status === MatchStatus.IN_PROGRESS) {
                                                                    if (pred && detailsData.match.homeScore !== null && detailsData.match.awayScore !== null) {
                                                                        points = calculatePoints(Number(pred.homeScore), Number(pred.awayScore), Number(detailsData.match.homeScore), Number(detailsData.match.awayScore), league.settings) + getGoalscorerBonus(detailsData.match.id, pred.playerPick);
                                                                    }
                                                                }
                                                                return (
                                                                        ...
                                                                        <div className="flex items-center gap-2 shrink-0 ml-2">
                                                                            <div className="w-16 flex flex-col items-center gap-0.5">
                                                                                <div className="text-lg font-black text-gray-700 dark:text-gray-300 tracking-wider whitespace-nowrap">
                                                                                    {pred ? <>{pred.homeScore} <span className="text-gray-300 dark:text-gray-600 text-sm">x</span> {pred.awayScore}</> : <span className="text-gray-300 dark:text-gray-600 text-sm font-normal">-</span>}
                                                                                </div>
                                                                                <div className="w-16 flex items-center justify-center gap-0.5 min-h-[14px]">
                                                                                    {pred?.playerPick ? (
                                                                                        <>
                                                                                            <Goal size={9} className="shrink-0 text-yellow-600 dark:text-yellow-400" />
                                                                                            <span className="text-[9px] font-bold text-yellow-700 dark:text-yellow-400 truncate max-w-[54px]">{pred.playerPick}</span>
                                                                                        </>
                                                                                    ) : null}
                                                                                </div>
                                                                            </div>
                                                            ...
                                                            <div className="bg-white dark:bg-gray-700/60 rounded-xl p-4 border border-gray-100 dark:border-gray-600 shadow-sm flex flex-col items-center justify-center text-center gap-3">
                                                                <div className="flex flex-col items-center">
                                                                    <span className="text-[10px] font-bold uppercase text-yellow-500 dark:text-yellow-400 mb-0.5 flex items-center gap-1.5 justify-center">
                                                                        <Trophy size={10} /> Placar mais palpitado
                                                                    </span>
                ...
                {selectedUserId && selectedUser && createPortal(
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setSelectedUserId(null)}>
                        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="flex-1 overflow-y-auto p-0 bg-gray-50/50 dark:bg-gray-800/50">{isLoadingHistory ? (
                                    <div className="flex flex-col items-center justify-center py-20 text-brasil-blue dark:text-blue-400">
                                        <Loader2 size={32} className="animate-spin mb-4" />
                ...
                <div className="flex items-center justify-between mb-4">
                    <button onClick={() => confirmNavigation(() => {
                        if (window.history.state?.idx > 0) {
                            ...
                            navigate('/brazil-games', { replace: true });
                        }
