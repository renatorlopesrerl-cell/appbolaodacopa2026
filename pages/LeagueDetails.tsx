    ...
    const renderAdminTab = () => {
        ...
        return (
            <div className="space-y-6">
                <div className={`rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800`}><div className="flex justify-between items-start"><div className="space-y-1"><h3 className={`font-bold text-lg flex items-center gap-2 text-gray-800 dark:text-white`}><Users size={20} className="text-gray-400" />Participantes</h3><p className={`text-sm text-gray-600 dark:text-gray-400`}>Limite atual: {league.participants.length} / {limit} participantes.</p></div></div><div className="w-full bg-black/10 dark:bg-white/10 rounded-full h-2.5 mt-4 overflow-hidden"><div className={`h-2.5 rounded-full ${league.participants.length >= limit ? 'bg-red-500' : 'bg-green-400'}`} style={{ width: `${Math.min(100, (league.participants.length / limit) * 100)}%` }}></div></div></div>
