...
export const LeagueDetails: React.FC = () => {
  ...
  const { 
    ...
    removeUserFromLeague, submitPredictions, sendLeagueInvite, updateLeague, loading 
  } = useStore();
  ...
  useEffect(() => {
      if (league) {
          ...
          // Note: we don't strictly sync isPrivate continuously to allow editing without jumps
          // but we set it initially.
      }
  }, [league]);
      ...
      <div className="space-y-6 pb-24">
        ...
        <div className="flex flex-col gap-4 pt-2">
            <div className="flex items-center justify-between px-1">
                ...
                <div className="text-xs text-gray-400 dark:text-gray-500 flex items-center gap-1"><Clock size={12} /> Horário de Brasília</div>
            </div>
        ...
        {filteredMatches.length === 0 ? (
            ...
            return (
                ...
                <div className="flex items-center justify-between mb-2">
                    <div className="flex flex-col items-center justify-center w-1/3 gap-2"><img src={getTeamFlag(match.homeTeamId)} alt={match.homeTeamId} className="w-12 h-9 object-cover rounded shadow-md"/><span className="text-center font-black text-sm md:text-base text-gray-900 dark:text-gray-100 leading-tight">{match.homeTeamId}</span></div>
                    ...
                    <div className="flex flex-col items-center justify-center w-1/3 gap-2"><img src={getTeamFlag(match.awayTeamId)} alt={match.awayTeamId} className="w-12 h-9 object-cover rounded shadow-md"/><span className="text-center font-black text-sm md:text-base text-gray-900 dark:text-gray-100 leading-tight">{match.awayTeamId}</span></div>
                </div>
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/20 p-5 rounded-xl border border-yellow-100 dark:border-yellow-800 flex flex-col items-center text-center hover:shadow-md transition-all"><div className="text-3xl font-black text-yellow-700 dark:text-yellow-400 mb-2">{league.settings?.winner ?? 5}</div><div className="font-bold text-gray-800 dark:text-gray-200 text-sm uppercase tracking-wide mb-2">Apenas Vencedor</div><p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">Acertou quem ganhou, mas errou o saldo.<br/><span className="text-yellow-800 dark:text-yellow-300 font-medium">Ex: Apostou 2x1 e foi 4x0.</span></p></div>
            <div className="bg-gray-50 dark:bg-gray-700/50 p-5 rounded-xl border border-gray-200 dark:border-gray-600 flex flex-col items-center text-center hover:shadow-md transition-all"><div className="text-3xl font-black text-gray-600 dark:text-gray-400 mb-2">{league.settings?.draw ?? 5}</div><div className="font-bold text-gray-800 dark:text-gray-200 text-sm uppercase tracking-wide mb-2">Empate (Não Exato)</div><p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">Acertou o empate, mas placar diferente.<br/><span className="text-gray-500 dark:text-gray-300 font-medium">Ex: Apostou 1x1 e foi 2x2.</span></p></div>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
