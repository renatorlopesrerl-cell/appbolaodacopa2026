...
export const LeagueDetails: React.FC = () => {
    ...
    return (
        <div>
            {toast && (
                <div
                    className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[99999] max-w-xs w-[90%] bg-white dark:bg-gray-800 border-l-4 shadow-2xl rounded-r-lg p-5 animate-in fade-in zoom-in-95 duration-300 flex items-start gap-3 transition-all ${toast.type === 'warning' ? 'border-yellow-500' : toast.type === 'success' ? 'border-brasil-green' : 'border-brasil-blue'
                        }`}
