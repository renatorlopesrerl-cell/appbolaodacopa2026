...
export const AdminMatchesPage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const success = await updateMatch(matchToSave);

      if (success) {
        setEditingMatch(null);
        if (addNotification) addNotification('Alterações Salvas', `O jogo ${matchToSave.homeTeamId} x ${matchToSave.awayTeamId} foi atualizado. Ranking atualizado.`, 'success');
      } else {
              ...
              <div className="grid grid-cols-2 gap-4 items-end bg-gray-50 dark:bg-gray-700 p-4 rounded-lg border border-gray-100 dark:border-gray-600">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-lg p-2 font-bold text-right focus:ring-2 focus:ring-brasil-blue outline-none transition-all text-sm text-gray-800 dark:text-white"
                  />
                </div>
                <div className="space-y-1">
