    ...
    const renderMatchBadge = (match: Match | any) => {
        ...
        return (
            <div className="flex gap-1 items-center">
                <span className={`text-[9px] ${champColorClass} px-1.5 py-0.5 rounded font-black border uppercase tracking-wider shadow-sm`}>
                    {champLabel}
                </span>
                {match.phase && (
                    <span className={`text-[9px] ${roundColorClass} px-1.5 py-0.5 rounded font-black border uppercase tracking-wider shadow-sm`}>
                                    ...
                                    return (
                                        <div className={`relative ${allowedCompetitions.length > 1 ? 'flex-1 md:flex-none md:w-auto min-w-[130px]' : 'w-full md:w-auto'}`}>
                                            <select value={palpitesSubPeriod} onChange={(e) => setPalpitesSubPeriod(e.target.value)} className={`w-full ${allowedCompetitions.length === 1 ? 'md:min-w-[160px]' : ''} appearance-none bg-gray-700 text-white border border-gray-600 text-xs font-bold rounded-lg focus:ring-brasil-blue focus:border-brasil-blue block p-2.5 pr-8`}>
                                                ...
                                                {Array.from({ length: 20 }, (_, i) => i + 19).map(r => (
                                ...
                                <div key={match.id} onClick={() => handleMatchClick(match, canClick, locked)} className={`bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border transition-all relative overflow-hidden ${isEdited ? 'border-brasil-yellow ring-1 ring-brasil-yellow' : 'border-gray-200 dark:border-gray-700'} ${canClick ? (locked ? 'cursor-pointer hover:border-brasil-blue dark:hover:border-blue-500 hover:shadow-md' : 'cursor-pointer hover:border-green-400 dark:hover:border-green-500 hover:shadow-md') : ''}`}>
                                    ...
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex flex-col items-center justify-center w-1/3 gap-2">
                                            <img referrerPolicy='no-referrer' src={getTeamFlag(String(match.home_team_id), teams)} alt={String(match.home_team_id)} onError={(e) => { (e.target as HTMLImageElement).src = LOGO_FALLBACK; }} className="w-10 h-10 object-contain drop-shadow-sm" />
                                            <span className="text-center font-black text-sm md:text-base text-gray-900 dark:text-gray-100 leading-tight">{getTeamNameForDisplay(match.home_team_id)}</span>
                                        ...
                                        <div className="flex flex-col items-center justify-center w-1/3 gap-2">
                                            <img referrerPolicy='no-referrer' src={getTeamFlag(String(match.away_team_id), teams)} alt={String(match.away_team_id)} onError={(e) => { (e.target as HTMLImageElement).src = LOGO_FALLBACK; }} className="w-10 h-10 object-contain drop-shadow-sm" />
                                            <span className="text-center font-black text-sm md:text-base text-gray-900 dark:text-gray-100 leading-tight">{getTeamNameForDisplay(match.away_team_id)}</span>
