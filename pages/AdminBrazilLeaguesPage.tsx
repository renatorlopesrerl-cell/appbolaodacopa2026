...
export const AdminBrazilLeaguesPage: React.FC = () => {
    ...
    const { currentUser, users, brazilLeagues, updateBrazilLeague, deleteBrazilLeague } = useStore();
    ...
    useEffect(() => { setCurrentPage(1); }, [leagueSearch]);
    ...
    const cycleLeaguePlan = async (leagueId: string, currentPlan: LeaguePlan) => {
        ...
        // Ensure isUnlimited flag is only true for VIP_UNLIMITED
    ...
    const handleDeleteLeague = async (leagueId: string) => {
        const success = await deleteBrazilLeague(leagueId);
        if (success) {
            showToast('Liga BR excluída com sucesso.');
        }
    ...
    const PlanBadge = ({ plan }: { plan: string }) => {
        if (plan === 'VIP_UNLIMITED') return <span className="bg-gray-900 text-yellow-400 border-yellow-900/50 px-2 py-1 rounded text-xs font-bold inline-flex items-center gap-1 border shadow-sm"><InfinityIcon size={12} strokeWidth={3} /> ILIMITADO</span>;
        if (plan === 'VIP_MASTER') return <span className="bg-gray-900 text-green-400 border-green-900/50 px-2 py-1 rounded text-xs font-bold inline-flex items-center gap-1 border shadow-sm"><Crown size={12} fill="currentColor" /> MASTER</span>;
        if (plan === 'VIP') return <span className="bg-gray-900 text-blue-400 border-blue-900/50 px-2 py-1 rounded text-xs font-bold inline-flex items-center gap-1 border shadow-sm"><Star size={12} fill="currentColor" /> TOP</span>;
        if (plan === 'VIP_BASIC') return <span className="bg-gray-900 text-gray-300 border-gray-700 px-2 py-1 rounded text-xs font-bold inline-flex items-center gap-1 border shadow-sm"><StarHalf size={12} fill="currentColor" /> BÁSICO</span>;
        return <span className="bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-600 px-2 py-1 rounded text-xs font-bold border inline-flex items-center gap-1">GRÁTIS</span>;
    };
    ...
    const Pagination = () => (
        <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-200 dark:border-gray-600">
            <div className="text-xs text-gray-500 dark:text-gray-400 font-medium">
                <span className="font-bold text-gray-700 dark:text-gray-200">{filteredLeagues.length}</span> ligas · página <span className="font-bold text-gray-700 dark:text-gray-200">{safePage}</span> de <span className="font-bold text-gray-700 dark:text-gray-200">{totalPages}</span>
            </div>
            <div className="flex items-center gap-2">
                <form className="flex items-center gap-1" onSubmit={(e) => { e.preventDefault(); const n = parseInt(pageInput); if (!isNaN(n)) goToPage(n); setPageInput(''); }}>
                    <span className="text-xs text-gray-500">Ir para:</span>
                    <input type="number" min={1} max={totalPages} value={pageInput} onChange={e => setPageInput(e.target.value)} placeholder={String(safePage)} className="w-14 text-center bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-800 dark:text-white rounded px-2 py-1 text-xs outline-none focus:border-brasil-blue" />
                </form>
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    ...
                    return <button key={page} onClick={() => goToPage(page)} className={`w-8 h-8 rounded-lg text-xs font-bold transition-colors border ${page === safePage ? 'bg-brasil-blue text-white border-brasil-blue' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>{page}</button>;
                })}
                <button onClick={() => goToPage(safePage + 1)} disabled={safePage >= totalPages} className="p-1.5 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"><ChevronRight size={16} /></button>
            </div>
        </div>
    );
            ...
            {toast && createPortal(
                ...
                >
                    <div
                        ...
                        className="bg-brasil-blue text-white px-8 py-5 rounded-xl shadow-2xl animate-in fade-in zoom-in-95 duration-300 font-bold max-w-[85vw] text-center border-2 border-white/20 backdrop-blur-sm"
                    >
                        {toast}
                    </div>
                </div>,
            ...
            <div className="mb-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-1">
                        Total: <span className="font-bold text-gray-700 dark:text-gray-200">{brazilLeagues.length}</span> ligas · exibindo {PAGE_SIZE} por página
                    </p>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-300 uppercase text-xs sticky top-0 z-10 border-b border-gray-200 dark:border-gray-600">
                            <tr>
                                ...
                                <th className="px-4 py-3">Nome da Liga BR</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {pagedLeagues.map((l, idx) => {
                                const plan = (l.settings as any)?.plan || (l.settings?.isUnlimited ? 'VIP_UNLIMITED' : 'FREE');
                                ...
                                return (
                                    <tr key={l.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        ...
                                        <td className="px-4 py-3 text-gray-500 dark:text-gray-400 text-xs">{adminUser?.name || l.adminId?.slice(0, 8) + '...'}</td>
                    ...
                    return (
                        <div key={l.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col gap-4 relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-start pl-3">
                                <div>
                                    <div className="flex items-center gap-1.5 mb-0.5"><span className="text-[10px] font-mono text-gray-400">#{globalIdx}</span></div>
                                    <h3 className="font-bold text-gray-800 dark:text-white text-lg leading-tight">{l.name}</h3>
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-1">
                                        <Users size={12} /> Admin: <span className="font-medium text-gray-700 dark:text-gray-300">{adminUser?.name || l.adminId?.slice(0, 8) + '...'}</span>
                                    </p>
                                </div>
                ...
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setDeletingLeagueId(null)}>
                    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-sm shadow-2xl border border-gray-200 dark:border-gray-700 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h3 className="font-bold text-lg text-gray-800 dark:text-white">Excluir Liga BR</h3>
                        </div>
                        <p className="text-sm text-gray-600 dark:text-gray-300 mb-2">
                            Tem certeza que deseja excluir a liga <strong>{brazilLeagues.find(l => l.id === deletingLeagueId)?.name}</strong>?
                        </p>
