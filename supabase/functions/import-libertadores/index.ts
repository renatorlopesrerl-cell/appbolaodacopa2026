...
const LIBERTADORES_LEAGUE_ID = 13;
const SEASON = "2026";

// Fases excluídas (primeiros turnos com centenas de times pequenos)
const EXCLUDED_ROUNDS = ['Qualification Round 1', 'Qualification Round 2', 'Qualification Round 3', 'Group Stage - 1', 'Group Stage - 2', 'Group Stage - 3', 'Group Stage - 4', 'Group Stage - 5', 'Group Stage - 6'];
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Importando jogos da Libertadores (Liga ${LIBERTADORES_LEAGUE_ID}, Temporada ${SEASON})...`);

    // 1. Buscar e salvar times
    const teamsRes = await fetch(
      `https://v3.football.api-sports.io/teams?league=${LIBERTADORES_LEAGUE_ID}&season=${SEASON}`,
    ...
    const fixturesRes = await fetch(
      `https://v3.football.api-sports.io/fixtures?league=${LIBERTADORES_LEAGUE_ID}&season=${SEASON}`,
      { headers: { 'x-apisports-key': API_FOOTBALL_KEY } }
    );
